...
class GISANS(object):
    ...
    def __call__(self, direct_beam=None):
        ...
        active_area_x = [0, 304]
        active_area_y = [0, 256]
        xtth = self.data_set.direct_pixel - np.arange(self.data_set.data.shape[0])[active_area_x[0]:
                                                                                   active_area_x[1]]
        ...
        phi = (np.arange(self.data_set.data.shape[1])[active_area_y[0]:
        ...
        wavelengths = np.asarray((lambda_edges[:-1] + lambda_edges[1:]) / 2.)
        ...
        self.p_f = k[np.newaxis, np.newaxis, P0:PN] * ((0. * phi) + np.sin(af)[:, np.newaxis])[:, :, np.newaxis]
        self.Qz = p_i + self.p_f

        raw = self.data_set.data[active_area_x[0]:active_area_x[1],
                                 active_area_y[0]:active_area_y[1],
                                 P0:PN]
        ...
        npoints, _, _ = np.histogram2d(self.Qy.flatten(), self.Qz.flatten(),
                                       bins=(50, 50))
